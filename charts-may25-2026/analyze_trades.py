...
# 3. Return Rate Over Time (Excluding Deposits)
ax3 = fig.add_subplot(gs[1, 1])

# Calculate return rate: (current_pnl / deposits_so_far) * 100
# Starting from first transaction, accumulate PnL and deposits separately
pnl_cumulative = 0
deposits_cumulative = 0
return_rates = []
market_times_x_axis_returns = []
market_times_set = set()
...
for idx, row in df.iterrows():
    ...
    # Only track unique market times (one entry per market window)
    if market_time not in market_times_set:
        market_times_set.add(market_time)
        
        # For this market time, accumulate all its transactions first
        for _, sub_row in df[df['marketName'] == market].iterrows():
            sub_amount = float(sub_row['usdcAmount'])
            sub_action = sub_row['action']
            
            if sub_action == "Buy":
                pnl_cumulative -= sub_amount
            elif sub_action == "Redeem":
                pnl_cumulative += sub_amount
            elif sub_action == "Deposit":
                deposits_cumulative += sub_amount
        
        # Calculate return rate: (pnl / deposits) * 100
        if deposits_cumulative > 0:
            return_rate = (pnl_cumulative / deposits_cumulative) * 100
        else:
            return_rate = 0
        
        return_rates.append(return_rate)
        market_times_x_axis_returns.append(market_time)

# Plot return rate
ax3.plot(range(len(market_times_x_axis_returns)), return_rates, linewidth=2.5, color='#F77F00', marker='o', markersize=5)
ax3.fill_between(range(len(market_times_x_axis_returns)), 0, return_rates, alpha=0.3, color='#F77F00')
ax3.axhline(y=0, color='black', linestyle='-', linewidth=1)

ax3.set_xticks(range(0, len(market_times_x_axis_returns), max(1, len(market_times_x_axis_returns)//10)))
ax3.set_xticklabels([market_times_x_axis_returns[i] for i in range(0, len(market_times_x_axis_returns), max(1, len(market_times_x_axis_returns)//10))], rotation=45, ha='right', fontsize=8)
ax3.set_ylabel('Return Rate (%)', fontsize=11, fontweight='bold')
ax3.set_title('Return Rate Over Time (Excluding Deposits)', fontsize=12, fontweight='bold')
ax3.grid(True, alpha=0.3)

# 4. Sharpe Ratio Over Time (using instantaneous returns)
ax4 = fig.add_subplot(gs[2, 0])

# Calculate instantaneous returns and Sharpe ratio for each market window
rf_rate = 0.04  # 4% risk-free rate annually
sharpe_ratios = []
market_times_x_axis_sharpe = []
market_times_set_sharpe = set()

# Build list of markets in order with their returns
market_order = []
for idx, row in df.iterrows():
    market = row['marketName']
    market_time = extract_market_time(market)
    if market_time not in market_times_set_sharpe:
        market_times_set_sharpe.add(market_time)
        market_order.append((market, market_time))

# Calculate instantaneous returns for each market
window_returns = []
cumulative_excess_return = 0
cumulative_variance = 0
cumulative_rf_cost = 0
num_windows = 0

for market, market_time in market_order:
    market_data = market_windows[market]
    buys = market_data["buys"]
    redeems = market_data["redeems"]
    
    total_buy_amount = sum(b['amount'] for b in buys)
    total_redeem_amount = sum(r['amount'] for r in redeems)
    
    if total_buy_amount > 0:
        # Instantaneous return for this market window
        window_return = (total_redeem_amount - total_buy_amount) / total_buy_amount if total_buy_amount > 0 else 0
        window_returns.append(window_return)
        
        # Accumulate for Sharpe ratio calculation
        cumulative_excess_return += window_return
        num_windows += 1

# Calculate Sharpe ratio at each step
cumulative_returns_list = []
cumulative_excess_returns_list = []
variance_list = []
sharpe_ratios_list = []

cumsum_return = 0
cumsum_squared_deviations = 0

for i, (market, market_time) in enumerate(market_order):
    if i < len(window_returns):
        ret = window_returns[i]
        cumsum_return += ret
        cumulative_returns_list.append(cumsum_return)
        
        # Risk-free rate expressed for the trading period (assuming ~5 min window)
        # We'll use the annual rate but normalize to cumulative
        excess_ret = cumsum_return - (rf_rate * (i + 1) / (252 * 288))  # 252 trading days, 288 5-min windows per day
        cumulative_excess_returns_list.append(excess_ret)
        
        # Calculate variance of returns so far
        if i == 0:
            variance = 0
        else:
            mean_return = cumsum_return / (i + 1)
            cumsum_squared_deviations = sum((window_returns[j] - mean_return) ** 2 for j in range(i + 1))
            variance = cumsum_squared_deviations / (i + 1) if (i + 1) > 0 else 0
        
        variance_list.append(variance)
        
        # Sharpe ratio = (cumulative_excess_return) / sqrt(cumulative_variance)
        std_dev = np.sqrt(variance) if variance > 0 else 0.0001
        sharpe = cumulative_excess_returns_list[-1] / std_dev if std_dev > 0 else 0
        sharpe_ratios_list.append(sharpe)
        
        market_times_x_axis_sharpe.append(market_time)

# Plot Sharpe ratio
ax4.plot(range(len(sharpe_ratios_list)), sharpe_ratios_list, linewidth=2.5, color='#6A4C93', marker='s', markersize=5)
ax4.fill_between(range(len(sharpe_ratios_list)), 0, sharpe_ratios_list, alpha=0.3, color='#6A4C93')
ax4.axhline(y=0, color='black', linestyle='-', linewidth=1)

ax4.set_xticks(range(0, len(market_times_x_axis_sharpe), max(1, len(market_times_x_axis_sharpe)//10)))
ax4.set_xticklabels([market_times_x_axis_sharpe[i] for i in range(0, len(market_times_x_axis_sharpe), max(1, len(market_times_x_axis_sharpe)//10))], rotation=45, ha='right', fontsize=8)
ax4.set_ylabel('Sharpe Ratio', fontsize=11, fontweight='bold')
ax4.set_title('Sharpe Ratio Over Time (RF=4%, Using Instantaneous Returns)', fontsize=12, fontweight='bold')
